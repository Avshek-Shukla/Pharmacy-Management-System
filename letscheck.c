...
//Declarations  a,b,c has been used To assign array value in different functions.
  char drugname[10][30];
  char dayname[32], drugtype[10];
  int hours, minutes, seconds, day, month, year; //for date and time
  int drugid[10], stock[10];//order function
  float tprice[10],price[10];
  int i, count, recnum[100], choice[10], quantity[10],date[10];
...
FILE *fptr;
   ...
   for(i=0; i<10; i++){
   	fscanf(fptr,"%d %c %s %f %d %d",&drugid[i],&drugtype[i],drugname[i],&price[i],&date[i],&stock[i]);
}
fclose(fptr);
...
order(){
    ...
    for (j=0; j<=9; j++){
	printf("%d\t\t   %c    \t%s\t\tRS %.2f  \t    %02d/%02d  \t\t  %d\n",drugid[j],drugtype[j],drugname[j],price[j],date[j]/100,date[j]%100,stock[j]);  
    } 	
    //Selection Starts
    printf("\nType Order Number:  ");
    b=getche();
